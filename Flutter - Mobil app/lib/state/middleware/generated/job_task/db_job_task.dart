...
class _UpsertResult {
  final JobTaskState model;
  final _Status status;
  _UpsertResult(this.model, this.status);
  factory _UpsertResult.inserted(JobTaskState model) =>
      _UpsertResult(model, _Status.INSERTED);
  factory _UpsertResult.updated(JobTaskState model) =>
      _UpsertResult(model, _Status.UPDATED);
}

class DBJobTask {
  static const TABLE_NAME = 'JobTask';

  // Returns the queried rows.
  static Future<Iterable<JobTaskState>> query(
    Database db,
    String username, {
    String jobNo,
    String jobTaskNo,
  ...
  static Future<JobTaskState> insert(
    Database db,
    String username,
    JobTaskState model, {
    int internalState,
  ...
  static Future<bool> update(
    ...
    if (username != null && username.isNotEmpty) {
      if (model.rowID != null) {
        final q = QueryArgs.fromJobTask(username, model);
        if (q.isCorrect) {
          if (internalState != null) {
            model = model.copyWith(internalState: internalState);
          }
