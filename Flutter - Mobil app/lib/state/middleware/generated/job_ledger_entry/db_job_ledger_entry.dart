...
class _UpsertResult {
  final JobLedgerEntryState model;
  final _Status status;
  _UpsertResult(this.model, this.status);
  factory _UpsertResult.inserted(JobLedgerEntryState model) =>
      _UpsertResult(model, _Status.INSERTED);
  factory _UpsertResult.updated(JobLedgerEntryState model) =>
      _UpsertResult(model, _Status.UPDATED);
}

class DBJobLedgerEntry {
  static const TABLE_NAME = 'JobLedgerEntry';

  // Returns the queried rows.
  static Future<Iterable<JobLedgerEntryState>> query(
    Database db,
    String username, {
    int entryNo,
  ...
  static Future<JobLedgerEntryState> insert(
    Database db,
    String username,
    JobLedgerEntryState model, {
    int internalState,
  }) async {
    if (username != null && username.isNotEmpty) {
      if (internalState != null) {
  ...
  static Future<bool> update(
    ...
    if (username != null && username.isNotEmpty) {
      if (model.rowID != null) {
        final q = QueryArgs.fromJobLedgerEntry(username, model);
        if (q.isCorrect) {
          if (internalState != null) {
            model = model.copyWith(internalState: internalState);
          }
