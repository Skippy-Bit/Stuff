...
class _UpsertResult {
  final ItemQuantityState model;
  final _Status status;
  _UpsertResult(this.model, this.status);
  factory _UpsertResult.inserted(ItemQuantityState model) =>
      _UpsertResult(model, _Status.INSERTED);
  factory _UpsertResult.updated(ItemQuantityState model) =>
      _UpsertResult(model, _Status.UPDATED);
}

class DBItemQuantity {
  static const TABLE_NAME = 'ItemQuantity';

  // Returns the queried rows.
  static Future<Iterable<ItemQuantityState>> query(
    Database db,
    String username, {
    String no,
  ...
  static Future<ItemQuantityState> insert(
    Database db,
    String username,
    ItemQuantityState model, {
    int internalState,
  }) async {
    if (username != null && username.isNotEmpty) {
      if (internalState != null) {
  ...
  static Future<bool> update(
    ...
    if (username != null && username.isNotEmpty) {
      if (model.rowID != null) {
        final q = QueryArgs.fromItemQuantity(username, model);
        if (q.isCorrect) {
          if (internalState != null) {
            model = model.copyWith(internalState: internalState);
          }
