...
class _UpsertResult {
  final LogState model;
  final _Status status;
  _UpsertResult(this.model, this.status);
  factory _UpsertResult.inserted(LogState model) =>
      _UpsertResult(model, _Status.INSERTED);
  factory _UpsertResult.updated(LogState model) =>
      _UpsertResult(model, _Status.UPDATED);
}

class DBLog {
  static const TABLE_NAME = 'Log';

  // Returns the queried rows.
  static Future<Iterable<LogState>> query(
    Database db,
    String username, {
    int entryNo,
  ...
  static Future<bool> update(
    ...
    if (username != null && username.isNotEmpty) {
      if (model.rowID != null) {
        final q = QueryArgs.fromLog(username, model);
        if (q.isCorrect) {
          if (internalState != null) {
            model = model.copyWith(internalState: internalState);
          }
