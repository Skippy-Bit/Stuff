...
class JobTaskUsagePage extends StatefulWidget {
  final JobState job;
  ...
  @override
  _JobTaskUsagePageState createState() => _JobTaskUsagePageState();
}

class _JobTaskUsagePageState extends State<JobTaskUsagePage> {
  ...
  Future<List<UsageItemModel>> getModels(BuildContext context) async {
    ...
    if (user != null && user.username.isNotEmpty) {
      ...
      where =
          'User = ? AND ((Command = ? AND Hash = ?) OR (Command = ? AND Hash LIKE ?))';
      whereArgs = [
        user.username,
        JobHour.COMMAND,
        JobHour.buildHash(widget.jobTask.jobNo, widget.jobTask.jobTaskNo),
        JobItemUsage.COMMAND,
        JobItemUsage.buildSqlLikeHash(
          jobNo: widget.jobTask.jobNo,
        ...
        StoreConnector<AppState, void>(
          ...
          builder: (context, _) => FutureBuilder<List<UsageItemModel>>(
                ...
                builder: (context, snapshot) {
                  ...
                  lines = lines
                      .where((line) => activeFilters.contains(line.typeAsInt))
                      .toList();
  ...
  List<Widget> _groupListItems(Iterable<UsageItemModel> lines) {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      result.add(line.buildUsageListTile(showJob: false, showTask: false));
    }
    return result;
  }
}
